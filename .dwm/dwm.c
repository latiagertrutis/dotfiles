...
Monitor *createmon(void)
{
    ...
    for (i = 0; i <= LENGTH(tags); i++) {
        ...
        for (j = 0; j < 3; j++) {
            m->pertag->areas[i][j].dir = MIN(dirs[j], ((int[]) {
                3, 1, 1
            }[j]));
...
{
    selmon->nmaster = selmon->pertag->nmasters[selmon->pertag->curtag] = MAX(selmon->nmaster + arg->i,
                                                                             0);
    arrange(selmon);
}
...
void tile(Monitor * m)
{
    ...
    if (ga->dir == DirHor || ga->dir == DirRotHor)
        ms = f * (m->ww - g), ss = m->ww - ms - g,
                              ma->x = ga->dir == DirHor ? 0 + m->ogappx : ss + g + m->ogappx, ma->y = 0 + m->ogappx,
                                                                                                  ma->fx = ma->x + ms - 2 * m->ogappx, ma->fy = m->wh - m->ogappx,
                                                                                                                                           sa->x = ga->dir == DirHor ? ms + g - m->ogappx : 0 + m->ogappx, sa->y = 0 + m->ogappx,
                                                                                                                                                                                                               sa->fx = sa->x + ss, sa->fy = m->wh - m->ogappx;
    else
        ms = f * (m->wh - g), ss = m->wh - ms - g,
                              ma->x = 0 + m->ogappx, ma->y = ga->dir == DirVer ? 0 + m->ogappx : ss + g + m->ogappx,
                                                         ma->fx = m->ww - m->ogappx, ma->fy = ma->y + ms - 2 * m->ogappx,
                                                                                         sa->x = 0 + m->ogappx, sa->y = ga->dir == DirVer ? ms + g - m->ogappx : 0 + m->ogappx,
...
void updatebars(void)
{
    ...
    for (m = mons; m; m = m->next) {
        ...
        m->barwin = XCreateWindow(dpy, root, m->wx + sp, m->by + vp, m->ww - 2 * sp, bh, 0,
...
pid_t winpid(Window w)
{
    ...
    if (XGetWindowProperty(dpy, w, XInternAtom(dpy, "_NET_WM_PID", 0), 0, 1, False, AnyPropertyType,
                           &type, &format, &len, &bytes, &prop) != Success || !prop) {
        return 0;
    }
